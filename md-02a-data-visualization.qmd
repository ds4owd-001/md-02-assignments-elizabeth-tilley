...
3.  Do not display (ignore) the outliers in the plot. **Note: Use a search engine or an AI tool to find the solution and add the link to the solution you have found.**
    ...
    -   this is my AI question and response: [https://chat.openai.com/share/cbc8796d-447d-4415-8804-b2de12911fca](#0)
...
ggplot(
  ...
  mapping = aes(x = continent, y = count, color = continent )) +
  geom_col()
...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here:
    ...
    -   geom_col needs you to tell it what the y is and geom_bar just knows? I'm not sure

```{r}
ggplot(
  data = gapminder_2007,
  mapping = aes(x = continent,  color = continent )) +
  geom_bar()
```
...
6.  Describe how the `geom_histogram()` function is similar to the `geom_bar()` function.
    ...
    -   it also doesn't need the y specified
...
7.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. Describe here what you observe:
    ...
    -   the first argument is the minimum value, the second is the maximum value, and the last is the size of the bin

```{r}
ggplot(
  data = gapminder_2007,
  mapping = aes(x = lifeExp,  color = continent )) +
  geom_histogram(col = "grey30", breaks = seq(0, 100, 5))
```
...
4.  What is stored in the `country_colors` object? Find out by executing the object in the Console (type it to the Console and hit enter). Do the same again, but with a question mark `?country_colors`.
    ...
    -   each country has a colour code associated with it

    -   adding the ? takes you to the help- very useful!

    -   I noticed that when I remove the "theme_minimal" nothing changes though
